		...
		jsonData.getData(dataUrl,"GET",{"data":JSON.stringify(param)},function(data){
			...
			if(data.code == 0){
				...
				if(data.data.consignee.consignee_d && data.data.consignee.consignee_d!="" ){				
					...
					oHtml += '<tr><td>[別送付先]ご注文者氏名ふりかな</td><td>'+(data.data.consignee.consignee_pinyin_d?data.data.consignee.consignee_pinyin_d:"")+'</td></tr>'
					oHtml += '<tr><td>[別送付先]電話番号</td><td>'+(data.data.consignee.tel_d?data.data.consignee.tel_d:"")+'</td></tr>'
					oHtml += '<tr><td>[別送付先]郵便番号</td><td>'+(data.data.consignee.zipcode_d?data.data.consignee.zipcode_d:"")+'</td></tr>'	
