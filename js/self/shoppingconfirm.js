...
let shoppingconfirm = {
	...
	getInfo:function(){
		...
		for(let i=0;i<list.length;i++){
			...
			jsonData.getData(dataUrl,"GET",{"goodsId":list[i].goods_id},function(data){
				console.log(data);
				if(data.code == 0){
					$(".m-shoppingconfirm-info-box-"+list[i].goods_id).find(".m-shoppingconfirm-method .text").text(data.data.payment.pay_name);
					$(".m-shoppingconfirm-info-box-"+list[i].goods_id).find(".m-shoppingconfirm-service .text").text(data.data.payment.send_info);
