		...
		jsonData.getData(dataUrl, "GET", { "data": JSON.stringify(param) }, function (data) {
			...
			if (data.code == 0) {
				...
				if (data.data.consignee.consignee_d && data.data.consignee.consignee_d != "") {
					...
					_oHtml2 += '<tr><td>[別送付先]ご注文者氏名ふりかな</td><td>' + (data.data.consignee.consignee_d_pinyin ? data.data.consignee.consignee_d_pinyin : "") + '</td></tr>';
					_oHtml2 += '<tr><td>[別送付先]電話番号</td><td>' + (data.data.consignee.tel_d ? data.data.consignee.tel_d : "") + '</td></tr>';
					_oHtml2 += '<tr><td>[別送付先]郵便番号</td><td>' + (data.data.consignee.zip_d ? data.data.consignee.zipcode_d : "") + '</td></tr>';
